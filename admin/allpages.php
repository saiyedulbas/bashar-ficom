...
$put = $_SESSION['email'];
	...
	$pagetitle = $_POST['juma'];
	
	$query = mysqli_query($connection,"DELETE FROM pages WHERE id = '$pagetitle'");
...
<button style="margin-left:20px" class="btn btn-danger">For any query, just contact Bashar at 01834663387 or see the documentation & site tour</button>
  ...
  <tbody>
		    ...
		    <?php 
	 $magur = mysqli_query($connection,"SELECT * FROM pages ");
	 while($row = mysqli_fetch_assoc($magur)){?> 
			 
				 <tr>
				 
				 <th scope="row"><a href="edit.php<?php echo $row['oldurl'];?>" class="btn btn-primary nuu"><?php echo $row['pagetitle']; ?></a></th>
				 ...
				 <input type="hidden" name="juma" value="<?php echo $row['id']; ?>"/>
				    <input type="submit" class="btn btn-primary demo" mimo="<?php echo $row['id']; ?>" value="delete" name="pagedelete"/>
				 </form>
				 </td>
