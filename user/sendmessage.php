...
<button style="margin-left:20px" class="btn btn-danger">For any query, just contact Bashar at 01834663387 or see the documentation & site tour</button>
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            ...
            <!-- Area Chuygklujhart -->
       Welcome <?php echo $take['firstname']; ?>. Manage your application. Send Message to other users
            <!-- Pie Chart -->
            
          </div>

         <ul class="messagesend">
	 <?php 
	 $third = $_SESSION['firstname'];
	 $first = mysqli_query($connection,"SELECT * FROM chat WHERE NOT firstname = '$third'");
	 while($row = mysqli_fetch_assoc($first)){?>
		 <li><a sakor="<?php echo $row['lastname']; ?>" janor="<?php echo $row['firstname']; ?>" class="tanor btn btn-primary" style="display:block;margin-top:20px;" href="#"><?php echo $row['firstname'].' '.$row['lastname']; ?></a></li>
	 <?php }
	 ?>
	 </ul>
         <div class="egg" style="display:none">
	<p class="eggkhao btn btn-primary">Type your message</p>
	<input type="text" class="eggb form-control" />
	</div>
